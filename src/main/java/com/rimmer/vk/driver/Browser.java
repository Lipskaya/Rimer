...
public class Browser implements WrapsDriver {
    ...
    //поле для хранения экземпляра com.rimmer.vk.driver.Browser класса
    ...
    // приватный конструктор, чтобы реализовать синглтон и не позволить создавать new Browser() вне этого класса
    //По примеру Степана
    private Browser() {
    ...
    //Метод, который надо вызывать вместо new com.rimmer.vk.driver.Browser(), чтобы получить экземпляр синглтора
